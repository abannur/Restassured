...
import api.endpoints.StudentEndpointProperties;
import api.payload.StudentInfo;

import io.restassured.response.Response;

public class StudentsTestsProperties {

	Faker faker;
	StudentInfo studentPayload;
	String userId;
	private static final Logger logger = LogManager.getLogger(StudentsTestsProperties.class);
	...
	public void testPostUser() {
		...
		Response response = StudentEndpointProperties.createUser(studentPayload);
	...
	public void testReadUserByUserName() {
		...
		Response response = StudentEndpointProperties.readUser(userId);
		response.then().log().all();
	...
	public void testUpdateUserByName() {
		...
		studentPayload.setLastName(faker.name().lastName());
		studentPayload.setSubjectId(2);
		Response response = StudentEndpointProperties.updateUser(userId, studentPayload);
		response.then().log().body();
		Assert.assertEquals(response.getStatusCode(), 200);
		Response responseAfterUpdate = StudentEndpointProperties.readUser(userId);
		Assert.assertEquals(responseAfterUpdate.getStatusCode(), 200);
	}

	@Test(priority = 4)
	public void testDeleteUser() {
		Response response = StudentEndpointProperties.deleteUser(userId);
		response.then().log().all();
		Assert.assertEquals(response.getStatusCode(), 200);
	}
